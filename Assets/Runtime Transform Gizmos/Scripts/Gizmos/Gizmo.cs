            ...
            if (RTInputDevice.Get.Device.IsButtonPressed(InputDeviceDragButtonIndex))
            {
                if (_activeDragSession != null && _activeDragSession.IsActive)
                {
                    if(_activeDragSession.Update())
                    {
                        ...
                        if (!relativeDragOffset.ExIsEquals(0.0f) && OffsetDragAxisModify != null)
                        {
						...
						// 오차를 보정한다 {
						var stOffset = _dragInfo.RelativeOffset;
						stOffset.x = (float)System.Math.Round(stOffset.x, 3);
						stOffset.y = (float)System.Math.Round(stOffset.y, 3);
						stOffset.z = (float)System.Math.Round(stOffset.z, 3);

						_dragInfo.RelativeOffset = stOffset;
						// 오차를 보정한다 }
